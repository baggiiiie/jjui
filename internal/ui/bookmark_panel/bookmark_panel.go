...
func (m *Model) DragStart(x, y int) bool {
	...
	if m.Parent == nil || m.Parent.Width == 0 || m.Parent.Height == 0 {
		return false
	}

	// Check if drag is on the resize handle (edge of the panel)
	if m.atBottom {
		// Panel is at bottom, handle is on the top edge
		if y < m.Frame.Min.Y || y > m.Frame.Min.Y+handleSize {
			return false
		}
	} else {
		// Panel is on the right, handle is on the left edge
		if x < m.Frame.Min.X || x > m.Frame.Min.X+handleSize {
			return false
		}
	}
...
func (m *Model) DragEnd(x, y int) tea.Cmd {
	return m.DragAware.DragEnd(x, y)
}
